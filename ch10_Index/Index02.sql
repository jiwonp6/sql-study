...
-- 클러스터형 인덱스의 루트 페이지 => 데이터(리프페이지 or 인터널페이지)의 참조 주소
-- 클러스터형 인덱스의 리프 페이지 => 실제 데이터가 저장 되어었음
...
-- * 혼합된 인덱스의 경우
-- 1) 보조 인덱스에서 컬럼명(이름) 기준으로 검색하여 루트 페이지에서 리프페이지로 탐색
	-- (리프 페이지 : name 기준으로 정렬되어 있으며 클러스터형 인덱스의 key 값을 저장)
-- 2) 리프 페이지에서 이름을 기준으로 클러스터형 인덱스의 키값(userId)을 확인한 후 클러스터형 루트 페이지로 이동
-- 3) 클러스터형 루트 페이지에서 키 값으로 검색한 후 원하는 결과를 찾음

-- WHERE 절에 인덱스를 사용한 컬럼의 이름이 있을 경우 해당 인덱스를 사용
